...
export class FormService {
    ...
    getLoggedInUserDetails(): Observable<any> {
        return this.http.get(`${this.appConfig.configuration.baseUrl}${ApiEndpoints.GET_LOGGED_IN_USER_DETAILS}`)
    }

    getPasswordEncryptionKey(): Observable<any> {
        return this.http.get(`${this.appConfig.configuration.baseUrl}${ApiEndpoints.FETCH_PASSWORD_ENCRYPTION_KEY}`, { responseType: 'text' })
    }

    eSignatureRequiredForSurvey(surveyResponseId: string): boolean {
        let requiresESignature: boolean = false
        let surveyResponse = this.surveyResponses.filter(ob => ob.id == surveyResponseId)[0]
        if (!surveyResponse || !surveyResponse.survey.configurationSettings) {
            return requiresESignature
        }
        let requiresESignatureSetting = surveyResponse.survey.configurationSettings.filter(ob => ob.settingName === SurveyConstants.REQUIRES_SIGNATURE)[0]
    ...
    authenticateUser(userDetails: any): Observable<any> {
        ...
        return this.http.post(`${this.appConfig.configuration.baseUrl}${ApiEndpoints.AUTHENTICATE_USER}`, userDetails, { params: params })
    }
    ...
    setRegistrationConfiguration(): Observable<any> {
        ...
        let apiEndpoint = Utility.stringFormat(ApiEndpoints.REGISTRATION_CONFIGURATION, [this.subjectService.loggedInSubject.preferredLanguage])
        return this.http.get<string>(`${this.appConfig.configuration.baseUrl}${apiEndpoint}`, { params: params }).pipe(map(result => {
    ...
    isSurveyReadOnlyMode(): boolean {
        ...
        let surveyResponse = this.surveyResponses.filter(ob => ob.id === this.selectedSurvey.id)
    ...
    isSurveyPast(response: SubjectSurveyResponse): boolean {
        return (response.timelineStatus.internalIdentifier === SurveyConstants.PAST_DUE_TIMELINE_STATUS ||
            response.completionStatus.internalIdentifier === SurveyConstants.COMPLETED_COMPLETION_STATUS ||
            response.completionStatus.internalIdentifier === SurveyConstants.SUBMITTED_COMPLETION_STATUS) && response.overrideStatus.internalIdentifier !== SurveyConstants.FORCED_OPEN_OVERRIDE_STATUS
    }

    getLegalAgreement(personId: string): Observable<any> {
        let apiEndpoint = Utility.stringFormat(ApiEndpoints.FETCH_LEGAL_STATEMENT, [personId, this.subjectService.loggedInSubject.preferredLanguage])
        return this.http.get<any>(`${this.appConfig.configuration.baseUrl}${apiEndpoint}`).pipe(map(result => {
    ...
    getAuthenticationRefreshToken(refreshToken: string): Observable<any> {
        let apiEndpoint = Utility.stringFormat(ApiEndpoints.FETCH_ACCESS_TOKEN_FROM_REFRESH_TOKEN, [refreshToken])
        return this.http.get<any>(`${this.appConfig.configuration.baseUrl}${apiEndpoint}`);
    }

    updateLegalAgreement(personId: string, requestPayload: LegalAgreementResponse[]): Observable<any> {
        let apiEndpoint = Utility.stringFormat(ApiEndpoints.UPDATE_LEGAL_STATEMENT, [personId])
    ...
    getOrganizationStudySites(organizationId: string): Observable<any> {
        let apiEndpoint = Utility.stringFormat(ApiEndpoints.ORGANIZATION_STUDY_SITES, [organizationId])
        return this.http.get<any>(`${this.appConfig.configuration.baseUrl}${apiEndpoint}`).pipe(map(result => {
    ...
    validateResponseFromBackend(apiEndpoint: string): Observable<any> {
        ...
        return this.http.get(`${this.appConfig.configuration.baseUrl}${apiEndpoint}`, { params: params })
    }
    ...
    getSurveyResponses() {
        ...
        // return this.http.get<SubjectSurveyResponse[]>(`${this.appConfig.configuration.baseUrl}${apiEndpoint}`, { params: params }).pipe(map(result => {
        // }))
        this.surveyResponses = this.getSortedSurveySectionResponses(surveyResponsesFromJson)
        return this.surveyResponses
    }

    // Sorts the section responses in ascending order based on ordinal
    getSortedSurveySectionResponses(surveyResponses: SubjectSurveyResponse[]): SubjectSurveyResponse[] {
    ...
    updateSurveyStatus(surveyResponseId: string, requestPayload: SurveyResponseStatus): Observable<any> {
        let loaderMessage = null
        if (requestPayload.completionStatusIdentifier === SurveyConstants.SUBMITTED_COMPLETION_STATUS) {
            loaderMessage = this.translate.instant("content.uploadSurvey")
        }
        else if (requestPayload.completionStatusIdentifier === SurveyConstants.PARTIAL_COMPLETION_STATUS) {
            // if (this.CookieService.get('app_downloaded_for_first_time') == 'YES') {
            //     loaderMessage = this.translate.instant("content.downloadSurvey")
            // } else {
                loaderMessage = this.translate.instant("content.loading")
        ...
        let apiEndpoint = Utility.stringFormat(ApiEndpoints.UPDATE_SURVEY_STATUS, [surveyResponseId])
        return this.http.post(`${this.appConfig.configuration.baseUrl}${apiEndpoint}`, requestPayload, { params: params })
    }
    ...
    getParamValues(subjectId: string, apiEndpoint: string): Observable<any> {
        ...
        return this.http.get(`${this.appConfig.configuration.baseUrl}${apiEndpoint}`);
    }

    setHtmlContent(contentPayload: string[], apiEndpoint: string): void {
        apiEndpoint = Utility.stringFormat(apiEndpoint, [this.subjectService.loggedInSubject.preferredLanguage])
        this.http.post<SectionHTMLContent[]>(`${this.appConfig.configuration.baseUrl}${apiEndpoint}`, contentPayload).subscribe(result => {
    ...
    setExternalHtmlContent(contentKey: string, apiEndpoint: string): void {
        this.getExternalHtmlContent(apiEndpoint).subscribe(result => {
            this.allStepsHtmlContent.push(new SectionHTMLContent(contentKey, result.legalStatementText))
            this.sectionHtmlContentSubject.next(this.allStepsHtmlContent)
    ...
    getSurveySectionConfiguration(sectionInternalId: string): any {
        if (this.configuration) {
            let sectionConf = this.configuration.sections.filter(ob => ob.identifier === sectionInternalId)
    ...
    setQuestionStepFormControl(step: any, formGroup: FormGroup): void {
        ...
        let defaultValue = this.getDefaultResponse(step.identifier, loglineRepeatKey)
        switch (step.answerFormat.type) {
            case "multipleValuePicker": {
                this.setMultipleValuePickerFormControl(step.answerFormat.valuePickers, formGroup, loglineRepeatKey)
                break
            }
            case "textChoice": {
                if (step.answerFormat.style === "multiple") {
                    this.setMultipleTextChoiceFormControl(step, formGroup, loglineRepeatKey)
                } else {
                    formGroup[formGroupKey] = new FormControl({ value: defaultValue, disabled: disabled })
                }
                break;
            }
            default: {
                ...
                formGroup[formGroupKey] = new FormControl({ value: defaultValue, disabled: disabled }, Validators.compose(validations))
            }
        }
    }
    ...
    getLogLineRepeatKey(step): number {
        ...
        } else if (step && Utility.extractKeyValueFromString("order", step.identifier)) {
            loglineRepeatKey = +Utility.extractKeyValueFromString("order", step.identifier)
        }
    ...
    setFormStepFormControl(step: any, formGroup: FormGroup): void {
        ...
        for (let item of step.formItems) {
            ...
            switch (item.answerFormat.type) {
                case "multipleValuePicker": {
                    this.setMultipleValuePickerFormControl(item.answerFormat.valuePickers, formGroup, loglineRepeatKey)
                    break
                }
                case "textChoice": {
                    if (item.answerFormat.style === "multiple") {
                        this.setMultipleTextChoiceFormControl(item, formGroup, loglineRepeatKey, step)
                    } else {
                        formGroup[formGroupKey] = new FormControl({ value: defaultValue, disabled: disabled })
                    }
                    break;
    ...
    setMultipleValuePickerFormControl(valuePickers: any, formGroup: FormGroup, loglineRepeatKey: number): void {
        let disabled = this.isSurveyReadOnlyMode()
        for (let picker of valuePickers) {
            let formGroupKey = `${picker.identifier}@${loglineRepeatKey}`
            let defaultValue = this.getDefaultResponse(picker.identifier, loglineRepeatKey)
            formGroup[formGroupKey] = new FormControl({ value: defaultValue, disabled: disabled })
        }
    }

    readSurveyFromJson(internalIdentifier: any, initiatingEventInterval: any, responseOrdinal: any): any {
        let surveys = this.translate.instant('survey')
        let surveyName = surveys[internalIdentifier]
        surveyName = Utility.stringFormat(surveyName, [responseOrdinal * initiatingEventInterval])
        return surveyName
    }
    ...
    setMultipleTextChoiceFormControl(step: any, formGroup: FormGroup, loglineRepeatKey: number, parentStep: any = null): void {
        ...
        let disabled = this.isSurveyReadOnlyMode()
        let formGroupKey = parentStep && !step.dictionary ? `${parentStep.identifier}.${step.identifier}@${loglineRepeatKey}` : `${step.identifier}@${loglineRepeatKey}`
        let formControls = []
        let selectedAnswers = this.getMultipleChoiceDefaultValues(step, loglineRepeatKey)
        for (let choice of step.answerFormat.textChoices) {
            let selectedAnswer = selectedAnswers.filter(answer => answer === choice.value)
            if (selectedAnswer.length > 0) {
                formControls.push(new FormControl({ value: choice.value, disabled: disabled }))
            } else {
    ...
    getFinalLoglineRepeatKey(step: any, parentStep: any = null): number {
        ...
        logLineRepeatKey = !logLineRepeatKey && parentStep ? this.getLogLineRepeatKey(parentStep) : logLineRepeatKey
    ...
    initializeSectionResponseAndProgress(): boolean {
        ...
        if (this.configuration.task === "form") {
            DataStorage.initializeFormSectionResponse(sectionResponseId, this.selectedSurvey.id)
        } else {
            let sectionResponse = DataStorage.getSurveySectionResponseValue(this.selectedSection.id, this.selectedSurvey.id)
            if ((!sectionResponse) || (sectionResponse && sectionResponse.itemDatas.length == 0)) {
                initialized = false
                DataStorage.initializeSurveySectionResponse(this.selectedSection.id, this.selectedSurvey.id, [])
            } else {
                DataStorage.initializeSurveySectionResponse(this.selectedSection.id, this.selectedSurvey.id, sectionResponse.itemDatas)
            }
        }
        // Section Progress is used to store the percentage the user has completed and the last visited question Id.
        DataStorage.initializeSectionProgress(sectionResponseId, this.selectedSurvey.id)
        return initialized
    }

    getMultipleChoiceDefaultValues(step: any, logLineRepeatKey: number): string[] {
        let selectedAnswers = []
        for (let textChoice of step.answerFormat.textChoices) {
            let textChoiceLoglineRepeatKey = +Utility.extractKeyValueFromString("order", textChoice.value)
            ...
            let defaultValue = this.getDefaultResponse(stepIdentifier, logLineRepeatKey)
    ...
    getDefaultResponse(stepIdentifier: string, logLineRepeatKey: number): string {
        ...
        if (this.configuration.task === "form") {
            responseValue = DataStorage.getFormQuestionResponse(sectionResponseId, stepIdentifier, this.selectedSurvey.id)
        } else {
            let response = DataStorage.getSurveyQuestionResponse(sectionResponseId, stepIdentifier, this.selectedSurvey.id, logLineRepeatKey)
    ...
    getFormStepValidations(step: any): any {
        ...
        if (step.answerFormat.maximumLength) {
            this.addMaximumLengthValidation(validations, step.answerFormat.maximumLength)
        }
        if (step.answerFormat.minimumLength) {
            this.addMinimumLengthValidation(validations, step.answerFormat.minimumLength)
        }
    ...
    submitQuestionResponse(data: SurveySectionResponseItemData[], progressPercentage: number): void {
        let surveyResponse: SurveyCompletion = new SurveyCompletion("N of 1 Mobile", this.subjectService.loggedInSubject.subjectId, this.subjectService.loggedInSubject.subjectIdentifier, "FOP Registry", "EPRO-NOVI-1", "24", this.subjectService.loggedInSubject.username, [])
        ...
        let apiEndpoint = Utility.stringFormat(ApiEndpoints.SUBMIT_QUESTION_RESPONSE, [this.selectedSurvey.id, this.selectedSection.id])
        this.http.post(`${this.appConfig.configuration.baseUrl}${apiEndpoint}`, requestPayload).subscribe({
    ...
    updateSurveySectionStatus(requestPayload: any, sectionId: string): Observable<any> {
        // let apiEndpoint = Utility.stringFormat(ApiEndpoints.UPDATE_SECTION_STATUS, [sectionId])
        // return this.http.post(`${this.appConfig.configuration.baseUrl}${apiEndpoint}`, requestPayload)
        return null
    }

    updateSubjectLanguage(subjectId: string, personId: string, preferredLanguage: string): Observable<any> {
        let apiEndpoint = Utility.stringFormat(ApiEndpoints.UPDATE_SUBJECT_LANGUAGE, [subjectId, personId, preferredLanguage])
        return this.http.post(`${this.appConfig.configuration.baseUrl}${apiEndpoint}`, null)
    }

    getSignatureDetails(): Observable<any> {
        return this.http.get<any>(`${this.appConfig.configuration.baseUrl}${ApiEndpoints.GET_SIGNATURE_DETAILS}`)
    }

    onCompleteRegistration(): void {
        let response = DataStorage.getFormResponse(this.selectedSection.id, this.selectedSurvey.id)
        if (response.OrganizationId && response.OrganizationId !== "" && !this.defaultStudySiteId) {
            this.getOrganizationStudySites(response.OrganizationId).subscribe(result => {
            ...
            response["StudySiteId"] = response.OrganizationId !== "" && this.defaultStudySiteId ? this.defaultStudySiteId : null
            let requestPayload = this.transformPayload(response)
            this.submitRegistrationForm(requestPayload)
        }
    }
    ...
    transformPayload(response: any): any {
        ...
        response["$type"] = "Nof1Health.Api.ViewModels.Subject.PersonSubjectViewModel, Nof1Health.Api"
    ...
    submitRegistrationForm(requestPayload: any): void {
        this.http.post(`${this.appConfig.configuration.baseUrl}${ApiEndpoints.REGISTRATION_SUBMIT}`, requestPayload).subscribe({
    ...
    fetchSurveySectionResponses(): Observable<any> {
        ...
        let apiEndpoint = Utility.stringFormat(ApiEndpoints.FETCH_SECTION_RESPONSES, [this.selectedSection.id])
        return this.http.get(`${this.appConfig.configuration.baseUrl}${apiEndpoint}`, { params: params })
    }

    public getFormInputModel(question: any, form: FormGroup, parentQuestion: any = null,): FormInput {
    ...
    public getSurveySectionCompletionStatus(sectionResponseId: string): string {
        ...
        let sectionResponse = this.selectedSurvey.sectionResponses.filter(ob => ob.id === sectionResponseId)
        if (sectionResponse.length > 0) {
            surveyStatus = sectionResponse[0].completionStatus ? sectionResponse[0].completionStatus.internalIdentifier : ""
        }
        return surveyStatus
    }
    getSubjectDetails(subjectId: string): Observable<SubjectProfile> {
        ...
        let apiEndpoint = Utility.stringFormat(ApiEndpoints.GET_SUBJECT_PROFILE_DETAILS, [subjectId])
        return this.http.get<SubjectProfile>(`${this.appConfig.configuration.baseUrl}${apiEndpoint}`, { params: params })
    }

    uploadSurveyData(file: File): Observable<any> {
        const formData = new FormData()
        formData.append("File", file, file.name)
        formData.append("SubjectIdentifier", this.subjectService.loggedInSubject.subjectIdentifier)
        let apiEndpoint = Utility.stringFormat(ApiEndpoints.UPLOAD_SURVEY_DATA, [this.selectedSection.id])
        return this.http.post(`${this.appConfig.configuration.baseUrl}${apiEndpoint}`, formData, { responseType: 'text' })
    }
}
