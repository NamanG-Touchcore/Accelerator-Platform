...
export class FormRenderer implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.formStepNavigatorService.childQuestionIndexSubject.subscribe(childQuestionIndex => {
  ...
  setupConfiguration(sectionInternalId: string): void {
    if (this.formService.configuration && this.formService.configuration.task === 'form') {
